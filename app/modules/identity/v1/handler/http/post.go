...
func (p *PostHandler) Add(ctx *gin.Context) {
	...
	// Lấy authorID từ context (sau khi middleware xác thực)
	// Giả sử user đã đăng nhập với userID = 1 (demo)
	authorID := uint64(1)

	postDetailResponse, err := p.postService.Add(postAddRequest, authorID)
	if err != nil {
		p.response.Out(ctx, err, nil)
		return
	}
...
func (p *PostHandler) List(ctx *gin.Context) {
	...
	posts, err := p.postService.ListByAuthor(authorID, limit, offset)
...
// @Param id path int true "number"
// @Success 200 {object} nil "success"
// @Router /identity/v1/post/{id}/publish [post]
func (p *PostHandler) Publish(ctx *gin.Context) {
	id := ctx.Param("id")
	if id == "" {
		p.response.Out(ctx, errors.New("not_allow"), nil)
		return
